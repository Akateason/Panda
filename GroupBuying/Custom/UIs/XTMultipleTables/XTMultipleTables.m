...
{
    FeaturesHandler *handlerCenter = (FeaturesHandler *)_list_handlers[_currentIndex] ;
...
//        [(XTTableViewRootHandler *)_list_handlers[0] centerHandlerRefreshing] ;
    ...
    if (self.allCount > 1)
    {
        ...
        if ([_list_handlers[self.allCount - 1] isKindOfClass:[FeaturesHandler class]]) {
            [((FeaturesHandler *)_list_handlers[self.allCount - 1]) table:_leftTable IsFromCenter:false] ;
        }
        if ([_list_handlers[1] isKindOfClass:[FeaturesHandler class]]) {
            [((FeaturesHandler *)_list_handlers[1]) table:_rightTable IsFromCenter:false] ;
        }
    }
...
- (void)resetLoopTimer
{
    int leftIndex   = (_currentIndex + self.allCount - 1) % self.allCount ;
    int rightIndex  = (_currentIndex + 1) % self.allCount ;
    if ([_list_handlers[leftIndex] isKindOfClass:[FeaturesHandler class]]) {
        [((FeaturesHandler *)_list_handlers[leftIndex]) table:_leftTable IsFromCenter:false] ;
    }
    if ([_list_handlers[rightIndex] isKindOfClass:[FeaturesHandler class]]) {
        [((FeaturesHandler *)_list_handlers[rightIndex]) table:_rightTable IsFromCenter:false] ;
    }
    
    if ([_list_handlers[_currentIndex] isKindOfClass:[FeaturesHandler class]]) {
        [((FeaturesHandler *)_list_handlers[_currentIndex]) table:_centerTable IsFromCenter:true] ;
    }
}
